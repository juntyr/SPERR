     ...
     // size_t count=0;
      ...
      for (size_t i = 0; i < total_vals; i++) {
        ...
        if ( !qoi->check_compliance(m_vals_orig[i],val_d)  ){
          ...
          if ( use_high_prec and m_vals_orig[i]!=val_d+offsets[i] and !hp ){
            std::cout<<"switch to high prec mode"<<std::endl;
            hp = true;
            m_vals_d = m_vals_orig;
            goto CMP_START;
      ...
      auto rtn = block_qoi_outlier_correction(use_high_prec);
      if( use_high_prec and rtn != RTNType::Good and !hp){
        std::cout<<"switch to high prec mode"<<std::endl;
        hp = true;
        m_vals_d = m_vals_orig;
        goto CMP_START;
...
auto sperr::SPECK_FLT::block_qoi_outlier_correction(bool use_high_prec) -> RTNType{
 ...
 // size_t count=0;
  ...
  size_t block_size_x = qoi_block_sizes[0],block_size_y = qoi_block_sizes[1], block_size_z = qoi_block_sizes[2];
